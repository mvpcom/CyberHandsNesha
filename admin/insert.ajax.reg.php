<?php
 ...
 if (isset($_POST['SessionSubject'], $_POST['SessionPlace'], $_POST['GD'], $_POST['SessionType'], $_POST['SessionHours'], $_POST['SessionReport'])) {
    ...
    $prep_stmt = "SELECT SID FROM session WHERE Subject = ? AND Place= ? AND Date= ? AND Type = ?  LIMIT 1";
    $stmt = $mysqli->prepare($prep_stmt);
 
   // check existing Session   
    if ($stmt) {
        $stmt->bind_param('ssss', $SessionSubject, $SessionPlace, $SessionDate, $SessionType);
        $stmt->execute();
        $stmt->store_result();
        ...
        if ($stmt->num_rows == 1) {
            // A user with this Session Parameters already exists
				$error_msg .='				
							<div class="alert alert-danger">
								<span class="glyphicon glyphicon-remove"></span><strong>شما قبلا این جلسه را ثبت کرده اید!</strong>
							</div>';
                        $stmt->close();
        }
                $stmt->close();
    ...
    if (empty($error_msg)) {
		...
		//$SessionDate=;
        if ($insert_stmt = $mysqli->prepare("INSERT INTO session (Date, Subject, Hours, Place, Type, Report) VALUES (?, ?, ?, ?, ?, ?)")) {
            $insert_stmt->bind_param('ssssss', $SessionDate, $SessionSubject, $SessionHours, $SessionPlace, $SessionType, $SessionReport);
		...
		echo "
							<div class='alert alert-success'>
								<strong><span class='glyphicon glyphicon-ok'></span>نشست/جلسه ی جدید با موفقیت ثبت شد!</strong>
							</div>
		";
/* 		echo "
							<div class='alert alert-success'>
